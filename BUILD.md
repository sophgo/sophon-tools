...
| 项目 | 路径 | deb 包名 | 默认安装位置 |
|------|------|----------|--------------|
| ssm | `source/pssm` | `ssm_<VER>_<ARCH>.deb` | `/opt/sophon/ssm/` + `/etc/systemd/system/ssm.service` |
| sophliteos | `source/psophliteos` | `sophliteos_<soc\|pcie>_<VER>.deb` | `/bin/sophliteos` + `/var/lib/sophliteos/dist` + `/etc/sophliteos/config/` |

两者都监听本机端口：ssm `:9779`（REST/WebSocket 后端），sophliteos `:8080`（Web 前端，反代 `/api/v1/*` → ssm）。
...
cd source/pssm
# arm64（设备，musl 静态链接）
bash build/build-deb-ssm.sh 2.0.0 arm64
# amd64（开发机/PCIE）
bash build/build-deb-ssm.sh 2.0.0 amd64
```

产物：`source/pssm/release/ssm_2.0.0_arm64.deb`

流程（`build/build-deb-ssm.sh`）：
1. `build/version.sh <VER>` 写 `build/version.txt`（`version|commit|buildtime`）
2. `build-ssm-arm64.sh` 交叉编译 musl 静态二进制（arm64）/ `build-ssm.sh` 宿主编译（amd64），ldflags 注入 version/commit/buildtime
3. 组装 deb 数据树（绝对路径布局）：
   - `/opt/sophon/ssm/bin/ssm`
   - `/opt/sophon/ssm/config/ssm.yaml`（**conffile**，升级保留用户改动）
   - `/etc/systemd/system/ssm.service`
4. `DEBIAN/`：`control`（Version/Architecture 注入）、`postinst`（建 `/var/lib/ssm /var/log/ssm`、enable+restart）、`prerm`（停服务）、`postrm`、`conffiles`、`md5sums`
5. `dpkg-deb --root-owner-group -b` 打包

### 版本号

- 由脚本第一个参数指定，默认 `2.0.0`
- 同时写入二进制（ldflags `ssm/global.version`）与 deb `control Version`，二者一致
- `--version` 走 `build/version.txt`，运行时 `ssm` 日志会打印 version/commit/buildtime

### 安装 / 升级（设备上）

```bash
# 首次安装
sudo dpkg -i ssm_2.0.0_arm64.deb

# 升级（保留用户改过的 ssm.yaml）
sudo dpkg -i --force-confold ssm_2.0.0_arm64.deb
# 升级且用包内新配置覆盖
sudo dpkg -i --force-confnew ssm_2.0.0_arm64.deb
```
...
- 配置 `/etc/sophliteos/config/sophliteos.yaml` 仅在不存在时拷入模板（**升级保留用户改动**）
...
该 SE7 设备分区：`/` overlay、`/opt` mmcblk0p6、`/data` mmcblk0p7（持久）。**OTA 刷机（`ota.sh sdcard.tgz`）会重写 eMMC 分区，把 `/opt`、overlay 上的 ssm/sophliteos 部署全部清掉**，设备回到出厂状态。每次 OTA 后需重装两个 deb。

刷机后 ssm DB 重置，默认 `admin/admin`（temp token，强制改密）。惯用 `admin123`。
...
- ssm `conffiles` 标注 `/opt/sophon/ssm/config/ssm.yaml`，升级由 dpkg 提示保留
