...
> 当前依据 v26.08.31 实际发版流程整理（MYS-841）。
...
Release 页面保持精简，**仅含子包版本清单表**：

- 表格列：工具 / 变更类型 / 旧版本 / 新版本 / **一句话**改动内容（改动人话精炼）。
- 不附加背景段落、发布形态、构建体系、发版记录、补丁小节等章节；相关细节以仓库 `RELEASE.md` 与 PR 记录为准。
...
- 工具代码如有发版后修复，按 §5.5 替换 release 资产；**Release 页面不追加补丁章节**（保持 §5.2 精简）。版本号是否 bump：工具代码/行为变更按 §2 bump（Z+1）；纯打包/元数据修复可不 bump（同版本替换资产）。
