...
// ============ M. 多 DNS(逗号分隔 → 数组,MYS-436)============
// M1:旧模式 v4 多 DNS 逗号分隔 → 解析为数组(渲染为 2 项而非 1 项逗号串)
case!(m1_old_mode_multi_dns, &["eth0","1.1.1.1","24","1.1.1.254","8.8.8.8,114.114.114.114"], [
    "v4.addrs=1.1.1.1/24", "v4.dns=8.8.8.8,114.114.114.114",
]);
// M2:多 DNS 逗号后带空格 → trim 后接受
case!(m2_multi_dns_with_space, &["eth0","1.1.1.1","24","1.1.1.254","8.8.8.8, 114.114.114.114"], [
    "v4.dns=8.8.8.8,114.114.114.114",
]);
// M3:4 元组 family1 的 dns 槽多 DNS
case!(m3_4tuple_multi_dns, &["eth0","1.1.1.1","24","1.1.1.254","8.8.8.8,114.114.114.114","192.168.2.0","24","1.1.1.254","100"], [
    "v4.dns=8.8.8.8,114.114.114.114", "routes.count=1",
]);
// M4:v6 多 DNS
case!(m4_v6_multi_dns, &["eth0","2001:db8::1","64","fe80::1","2001:4860:4860::8888,2001:4860:4860::8844"], [
    "v6.addrs=2001:db8::1/64", "v6.dns=2001:4860:4860::8888,2001:4860:4860::8844",
]);
// M5:多 DNS 混族(v4+v6 混合)在 4 元组模式被拦——各后端(nmcli ipv4.dns/ipv6.dns)按族隔离,
//    混族会在运行时失败,故解析层直接拒绝(旧模式含 ':' 串会触发 family2 跳转,同样被拦)
case_err!(m5_mixed_v4_v6_dns, &["eth0","1.1.1.1","24","1.1.1.254","8.8.8.8,2001:4860:4860::8888","192.168.2.0","24","1.1.1.254","100"], "single-family");
// M6:dhcp 族 dns 槽多 DNS(旧模式 dhcp 尾部槽)
case!(m6_dhcp_multi_dns, &["eth0","dhcp","","","8.8.8.8,1.1.1.1"], [
    "v4.is_dhcp=true", "v4.dns=8.8.8.8,1.1.1.1",
]);
// M7:多 DNS 中某项非法 → 拒绝(报该项)
case_err!(m7_multi_dns_bad_entry, &["eth0","1.1.1.1","24","1.1.1.254","8.8.8.8,999.1.1.1"], "invalid DNS server '999.1.1.1'");
// M8:多 DNS 空项(连续逗号)→ 拒绝
case_err!(m8_multi_dns_empty_entry, &["eth0","1.1.1.1","24","1.1.1.254","8.8.8.8,,1.1.1.1"], "empty");
// M9:多 DNS 尾逗号 → 拒绝
case_err!(m9_multi_dns_trailing_comma, &["eth0","1.1.1.1","24","1.1.1.254","8.8.8.8,"], "empty");
// M10:多 DNS 项含 '/' → 拒绝
case_err!(m10_multi_dns_slash_entry, &["eth0","1.1.1.1","24","1.1.1.254","8.8.8.8,1.1.1.1/24"], "invalid DNS server '1.1.1.1/24'");
// M11:逗号后仅空白(空项)→ 拒绝
case_err!(m11_multi_dns_blank_entry, &["eth0","1.1.1.1","24","1.1.1.254","8.8.8.8, ,1.1.1.1"], "empty");
// M12:4 元组 dhcp 后接多 DNS 时,逗号串被当组地址 → 拒绝(而不是透传)
case_err!(m12_dhcp_4tuple_multi_dns_misuse, &["eth0","dhcp","","","","8.8.8.8,1.1.1.1","24","1.1.1.254","100"], "invalid IPv4 address '8.8.8.8,1.1.1.1'");

// M13:v6 family 的 dns 槽混入 v4 DNS → 拒绝
case_err!(m13_v6_family_v4_dns, &["eth0","2001:db8::1","64","fe80::1","8.8.8.8,114.114.114.114"], "expected IPv6 address");
