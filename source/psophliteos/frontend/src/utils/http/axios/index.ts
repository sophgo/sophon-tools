...
const transform: AxiosTransform = {
  ...
  transformResponseHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    if (hasSuccess) {
      // 除了get请求，其他请求操作成功给予提示
      // @ts-ignore
      if (res.config.method !== 'get' && !res.config.requestOptions.noSuccessMessage) {
  ...
  requestInterceptors: (config, options) => {
    ...
    (config as Recordable).headers['Accept-Language'] = localeStore.getLocale;
    return config;
  },
