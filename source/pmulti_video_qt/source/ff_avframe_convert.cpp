...
static void bmBufferDeviceMemFree2(void *opaque, uint8_t *data) { return; }
...
int avframe_to_bm_image(bm_handle_t &bm_handle, AVFrame &in, bm_image &out) {
  ...
  case AV_PIX_FMT_GBRP:
    ...
    break;
  default:
    printf("unsupported format, only gray,nv12,yuv420p,nv16,yuv422p "
  ...
  if (in.channel_layout == 101) { /* COMPRESSED NV12 FORMAT */
    ...
    bm_image cmp_bmimg;
    bm_image_create(bm_handle, in.height, in.width, FORMAT_COMPRESSED,
    ...
    bm_image_create(bm_handle, in.height, in.width, FORMAT_YUV420P,
                    DATA_TYPE_EXT_1N_BYTE, &out);
    // bm_image_dev_mem_alloc(out);
    if (mem_flags == USEING_MEM_HEAP2 &&
        bm_image_alloc_dev_mem_heap_mask(out, USEING_MEM_HEAP2) != BM_SUCCESS) {
      mem_flags = USEING_MEM_HEAP1;
    }
    if (mem_flags == USEING_MEM_HEAP1 &&
        bm_image_alloc_dev_mem_heap_mask(out, USEING_MEM_HEAP1) != BM_SUCCESS) {
    ...
    if (plane == 1) {
      ...
      if ((0 == in.height) || (0 == in.width) || (0 == in.linesize[4]) ||
          (0 == in.linesize[5]) || (0 == in.data[4]) || (0 == in.data[5])) {
        return BM_ERR_PARAM;
      }
      ...
      if ((0 == in.height) || (0 == in.width) || (0 == in.linesize[4]) ||
          (0 == in.linesize[5]) || (0 == in.linesize[6]) || (0 == in.data[4]) ||
          (0 == in.data[5]) || (0 == in.data[6])) {
        return BM_ERR_PARAM;
      }
    ...
    bm_image_create(bm_handle, in.height, in.width, bm_format,
                    DATA_TYPE_EXT_1N_BYTE, &out, stride);
...
int avframe_to_bm_image_by_out(bm_handle_t &bm_handle, AVFrame &in,
                               bm_image &out) {
  ...
  if (in.channel_layout == 101) { /* COMPRESSED NV12 FORMAT */
    ...
    bmcv_image_storage_convert(bm_handle, 1, &cmp_bmimg, &out);
    bm_image_detach(cmp_bmimg);
    bm_image_destroy(cmp_bmimg);
  } else {
    int stride[3];
    bm_image_format_ext bm_format;
    ...
    bm_image cmp_bmimage;
    bm_format = (bm_image_format_ext)map_avformat_to_bmformat(in.format);
    bm_image_create(bm_handle, in.height, in.width, bm_format,
                    DATA_TYPE_EXT_1N_BYTE, &cmp_bmimage, stride);
    ...
    bm_image_attach(cmp_bmimage, input_addr);
    bmcv_image_storage_convert(bm_handle, 1, &cmp_bmimage, &out);
    bm_image_detach(cmp_bmimage);
    bm_image_destroy(cmp_bmimage);
  }
  return BM_SUCCESS;
}
...
int bm_image_to_avframe(bm_handle_t &bm_handle, bm_image *in, AVFrame *out) {
  ...
  out->format =
      (AVPixelFormat)map_bmformat_to_avformat(ImgOut->bmImg->image_format);
