	...
	if [ $3 -eq 1 -o $3 -eq 2 ]; then
		if [ -f ${PART_IMAGE_FILE_NAME[$2]} -a "${P_FLAG[$2]}" == "true" ]; then
			...
			if [ -f ${PART_COMPRESS_FILE_NAME[$2]} ]; then
				...
				# edit by bmsec start
				if [ $3 -eq 1 ]; then
					sudo tar -I ${PIGZ_GZIP_COM} -xf ${PART_COMPRESS_FILE_NAME[$2]} --no-same-owner -C $RECOVERY_DIR/$MOUNT_DIR-$2
				else
					sudo tar -I ${PIGZ_GZIP_COM} -xf ${PART_COMPRESS_FILE_NAME[$2]} -C $RECOVERY_DIR/$MOUNT_DIR-$2
				fi
				# edit by bmsec end
				sync
