...
# f2fs 分区（xml 里 format="2" + fstype="f2fs"）的 mkfs 特性，与 socbak.sh、SDK 打包链
...
function parse_partition_xml()
{
	...
	# 可选属性 fstype="ext4|f2fs"（缺省 ext4），只对 format="2" 的分区有意义：
	# format 表示"分区类别"（0=raw/1=FAT32/2=带文件系统的分区），fstype 才是具体文件系统。
	# 逐行解析而不是 grep -P 一把抓：属性是可选的，缺省时 grep 会少一行，导致下标错位
	# （把 A 分区的 fstype 安到 B 分区上）。
	local _line _fstype
	PART_FSTYPE=()
	while IFS= read -r _line; do
		...
		_fstype=""
		if [[ "${_line}" =~ fstype=\"([A-Za-z0-9_]+)\" ]]; then
			_fstype="${BASH_REMATCH[1]}"
		fi
		PART_FSTYPE+=("${_fstype:-ext4}")
	done < "${PARTITION_FILE}"
	if [ "${#PART_FSTYPE[@]}" != "${#LABELS[@]}" ]; then
		panic "fstype parse mismatch: ${#PART_FSTYPE[@]} values for ${#LABELS[@]} partitions in ${PARTITION_FILE}"
	fi

	local _tool
	for i in $(seq 0 $[${#LABELS[@]}-1]); do
		case "${PART_FSTYPE[$i]}" in
			ext4|f2fs) ;;
			*) panic "partition ${LABELS[$i]}: unsupported fstype \"${PART_FSTYPE[$i]}\" (expect ext4 or f2fs)" ;;
		esac
		if [ "${PART_FORMAT[$i]}" != "2" ] && [ "${PART_FSTYPE[$i]}" != "ext4" ]; then
...
function create_top_script()
{
	...
	script_update ""
	script_update "setenv update_all 1"
	script_update "led status off"
...
function create_fip_script()
{
	CURRENT_SCRIPT=${RECOVERY_DIR}/${FIPCMD_SCRIPT}.cmd
	cp /dev/null ${CURRENT_SCRIPT}

	if [ ! -e ${SOURCE_FILES_PATH}/fip.bin ]; then
		echo "no fip.bin"
	else
		cp ${SOURCE_FILES_PATH}/fip.bin ${RECOVERY_DIR}
		touch ${RECOVERY_DIR}/BOOT
		...
		script_update "else"
		script_update ""
		script_update "echo skip fip flash."
		script_update "fi"
		script_update ""
...
# arguments:
# $3: file system type: 0 for raw partition; 1 for FAT32; 2 for a filesystem partition
#     (format="2" 的具体文件系统由 xml 的 fstype 属性决定：缺省 ext4，可为 f2fs)
...
function gen_partition_img()
{
	...
	if [ "$part_format" = "2" ] && [ "${PART_FSTYPE[$part_number]}" != "f2fs" ]; then
		local resize_flag="1"
	else
		local resize_flag="0"
	fi
