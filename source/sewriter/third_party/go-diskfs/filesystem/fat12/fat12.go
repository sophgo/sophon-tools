...
type FileSystem struct {
	...
	// PATCH(setf) 5: 是否已经整张 FAT 写过一次。第一次必须整张写 (清掉卡上残留的
	// 旧 FAT 项, 否则空闲簇会被旧数据标成"已占用"), 之后只回写变化的部分。
	fatWrittenOnce bool
}
...
// fatDirtyTracker PATCH(setf) 5: 由支持"只回写脏区间"的 FAT 表实现 (目前是 FAT32)。
// 未实现的表 (FAT12/FAT16) 走原来的整张写路径, 行为不变。
type fatDirtyTracker interface {
	DirtyRange() (from, to uint32, ok bool)
	BytesRange(from, to uint32) []byte
	ClearDirty()
}
...
func (fs *FileSystem) defaultWriteFAT() error {
	...
	tracker, tracked := fs.table.(fatDirtyTracker)
	if !tracked || !fs.fatWrittenOnce {
		// 首次 (或 FAT12/16): 整张照写
		fatBytes = fs.table.Bytes()
	} else {
		from, to, dirty := tracker.DirtyRange()
		if !dirty {
			return nil // 没有任何表项变化 → 一个字节都不用写
		}
		fatBytes = tracker.BytesRange(from, to)
		byteOff = int64(from) * 4
	}
	if _, err := writableFile.WriteAt(fatBytes, int64(fs.fatPrimaryStart)+fs.start+byteOff); err != nil {
		return fmt.Errorf("unable to write primary FAT: %w", err)
	}
	if _, err := writableFile.WriteAt(fatBytes, int64(fs.fatSecondaryStart)+fs.start+byteOff); err != nil {
		return fmt.Errorf("unable to write secondary FAT: %w", err)
	}
	if tracked {
		fs.fatWrittenOnce = true
		tracker.ClearDirty()
	}
	return nil
}
