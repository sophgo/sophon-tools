...
// PATCH(setf) 6: 原实现用 github.com/elliotwutingfeng/asciiset (其新版本要求 Go 1.24+,
// 而 Win7 只能用 Go <= 1.20 编译)。这里换成等价的字符集判断, 去掉该依赖。
const validShortNameCharset = "!#$%&'()-0123456789@ABCDEFGHIJKLMNOPQRSTUVWXYZ^_`{}~"

// validShortNameChar 判断一个**字节**是否属于 FAT 8.3 短名合法字符集
func validShortNameChar(c byte) bool {
	return c < 0x80 && strings.ContainsRune(validShortNameCharset, rune(c))
}

// validShortNameRune 判断一个**字符**是否可原样进 8.3 短名。
// 必须先卡 ASCII 再查字符集 —— 否则 byte(val) 的截断会让"低字节恰好合法"的非 ASCII
// 字符 (中 U+4E2D → 0x2D '-') 蒙混过关, 后面算 8.3 校验和时才失败。
func validShortNameRune(r rune) bool {
	return r <= 0x7f && strings.ContainsRune(validShortNameCharset, r)
}
...
func longFilenameEntryFromBytes(b []byte) (string, error) {
	...
	for i := 0; i < maxCharsLongFilename; i++ {
		...
		// PATCH(setf): 0x0000 是结束符, 0xFFFF 是槽内填充 —— 两者都应停止,
		// 否则别的工具(或旧版本)写出的多槽 LFN 会被还原成尾部带乱码的名字。
		if val == 0 || val == 0xFFFF {
			break
		}
		r = append(r, rune(val))
...
func calculateSlots(s string) int {
	// PATCH(setf): 原实现用 len(s) —— **字节数**。LFN 每个槽放 13 个 UCS-2 字符,
	// 一个汉字占 3 字节但只占 1 个字符位, 于是中文名的槽数被高估:
	// 多出来的槽用 0xFFFF 填满, 而读取端把 0xFFFF 当真实字符, 还原出的名字尾部
	// 多出一串乱码 → 按名字查文件必然失败。改成按 rune 数计算。
	sLen := len([]rune(s))
...
// returns shortName, extension, isLFN, isTruncated, isRewritten
//
//	isLFN : was there an LFN that had to be converted
//	isTruncated : was the shortname longer than 8 chars and had to be converted?
//	isRewritten : were any characters of the name actually replaced/dropped (as
//	              opposed to a plain upper-casing)?  PATCH(setf): callers need this
//	              to tell "fip.bin" (a perfectly legal 8.3 name that merely needs an
//	              LFN to preserve its lower case) apart from "中文.bin" (where the
//	              stem is mangled into underscores and may collide with siblings).
func convertLfnSfn(name string) (shortName, extension string, isLFN, isTruncated, isRewritten bool) {
	...
	if lastDot > -1 {
		...
		if extension != strings.ToUpper(rawExtension) {
			isRewritten = true
		}
	}
	...
	if shortName != strings.ToUpper(rawShortName) {
		isRewritten = true
	}
	...
	return shortName, extension, isLFN, isTruncated, isRewritten
}
...
func uCaseValid(name string) string {
	...
	for _, val := range r {
		switch {
		// PATCH(setf): 原实现是 validShortNameCharacters.Contains(byte(val)) —— 把 rune
		// 截断成 8 位再判合法性, 于是低字节恰好落在合法集里的**非 ASCII 字符**会被原样
		// 保留 (例: 中 U+4E2D → byte 0x2D 是合法的 '-'), 随后 8.3 校验和计算因
		// "non-ASCII character in name" 失败, 中文名文件根本写不进去。
		// 修法: 先要求确实是 ASCII (val <= 0x7f) 再查合法字符集。
		case validShortNameRune(val):
			r2 = append(r2, val)
