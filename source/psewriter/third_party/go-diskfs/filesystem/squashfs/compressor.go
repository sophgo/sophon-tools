...
func (c *CompressorLzo) decompress(_ []byte) ([]byte, error) {
	// PATCH(setf) 6: 上游用 github.com/anchore/go-lzo 解 LZO —— 该库要求 Go 1.24,
	// 而 Windows 7 只能用 Go <= 1.20 编译。本工具只读 FAT 分区, 用不到 LZO squashfs,
	// 因此这里直接返回"不支持", 从而去掉整个依赖。
	return nil, fmt.Errorf("LZO 压缩的 squashfs 不受支持 (Win7 构建去掉了该依赖)")
}
