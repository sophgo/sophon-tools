...
// TestLoadRealRepoConfig 覆盖本地 ./config/ssm.yaml 回退路径：
// 显式从仓库内置的 config 目录加载，验证 port=9779 auth=true。
func TestLoadRealRepoConfig(t *testing.T) {
	ok := LoadFromDir(".")
	if !ok {
		t.Skip("no ssm.yaml in CWD; skip real-config path test")
	}
	v := Conf.GetViper()
	if v.GetString("server.port") != "9779" {
		t.Fatalf("expected default port 9779 from repo ssm.yaml, got %s", v.GetString("server.port"))
	}
	if v.GetBool("server.auth") != true {
		t.Fatal("expected auth=true from repo ssm.yaml")
	}
}

// TestLoadConfigSSMConfEnv 覆盖 SSM_CONF 环境变量优先回退路径：
// 设置 SSM_CONF 指向自定义目录，调用 LoadConfig()，验证 port 为自定义值。
func TestLoadConfigSSMConfEnv(t *testing.T) {
	...
	os.Setenv("SSM_CONF", dir)
	defer os.Unsetenv("SSM_CONF")

	LoadConfig()
	v := Conf.GetViper()
	if v.GetString("server.port") != "7777" {
		t.Fatalf("expected port 7777 from SSM_CONF dir, got %s", v.GetString("server.port"))
	}
}
...
// TestAlarmThresholdFromYaml 验证从 ssm.yaml 读取 alarmThreshold 覆盖默认值。
...
func TestAlarmThresholdPersistence(t *testing.T) {
	dir := t.TempDir()
	yamlPath := filepath.Join(dir, "ssm.yaml")
	yaml := []byte(`server:
  port: "7777"
alarmThreshold:
  boardTemperature: 90
