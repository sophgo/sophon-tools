...
CGO_ENABLED=1 GOOS=linux GOARCH=arm64 \
  ...
  go build -trimpath \
    ...
    -ldflags '-s -w -X ssm/global.version='"${VERSION}"' -X ssm/global.gitCommit='"${COMMIT}"' -X ssm/global.buildTime='"${BUILDTIME}"' -linkmode external -extldflags "-static"' \
  -o ssm-arm64 . \
  || { echo "ERROR: arm64 构建失败"; exit 1; }

# 静态链接门禁
file ssm-arm64 | grep -q 'statically linked' \
...
echo "built release/ssm (arm64, musl static)"
